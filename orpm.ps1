...
function show_menu() {
    ...
    add_line "build" "   # 打包 nginx app"
    add_line "update" "  # 升级 orpm"
    add_line "install" " # 安装 libs"
    add_line "rocks" "   # 执行 luarocks"
    add_line
    add_line "install" " killsen/openresty-lua-types -d    # 开发时依赖"
    ...
    add_line "install" " bungle/lua-resty-template@master  # 仓库主分支版"
    add_line
...
switch ($args[0]) {
    ...
    "build"   { . $PSScriptRoot\scripts\build.ps1   }
